    ...
    public void run() {
        try {
            ...
            while (loop) {
				...
				else if (bufferData.get(0).equals("ADDFILE")) {
                    if (bufferData.size() < 2) {
                        System.out.println("Error: Null Filename.");
                        out.writeUTF("ERROR NULL FILENAME");
                    } else {
                        String filename = bufferData.get(1);
                        File file = new File(this._path.getRootPath() + "/" + filename);

                        if (file.exists()) { //verifica se o arquivo existe
                            System.out.println("Error: File " + filename + " already exists.");
                            out.writeUTF("ERROR FILE EXISTS");
                        } else {
                            try {
                                //cria um novo arquivo
                                file.createNewFile();
                                out.writeUTF("ADDFILE_OK"); //informa o cliente que ele pode enviar o arquivo

                                FileOutputStream fileOutputStream = new FileOutputStream(this._path.getRootPath() + "/" + filename);

                                // recebe o arquivo
                                byte[] bufferB = new byte[4096];
                                int bytesRead;
                                while ((bytesRead = in.read(bufferB)) > 0) {
                                    fileOutputStream.write(bufferB, 0, bytesRead);
                                ...
                                System.out.println("File " + filename + " added by user " + userName);
                            } catch (IOException ioe) {
                                System.out.println("Error adding file: " + ioe.getMessage());
                                out.writeUTF("ERROR");
                            }
                        }
                    }
                ...
                // deleta um arquivo
                    ...
                    if (bufferData.size() < 2) {
                        ...
                        if (!file.exists()) { //verifica se o arquivo existe
                            ...
                            // deleta o arquivo e verifica se foi deletado
                            if (file.delete()) {
                                System.out.println("File " + filename + " deleted by user " + userName);
                                out.writeUTF("SUCCESS");
                            } else {
                ...
                // lista os arquivos na pasta
                else if (bufferData.get(0).equals("GETFILESLIST")) {
                    File directory = new File(this._path.getRootPath() + "/"); //abre o diretorio

                    // procura no diretorio os arquivos
                    if (directory.isDirectory()) {
                        ...
                        out.writeUTF(fileList.toString()); //envia para o cliente os arquivos
                        System.out.println("User: " + userName + " requested file list.");
                    } else {
                        out.writeUTF("ERROR");
                    }
                }
                // envia um arquivo para o cliente
				else if (bufferData.get(0).equals("GETFILE")) {
                    if (bufferData.size() < 2) {
                        System.out.println("Error: Null Filename.");
                        out.writeUTF("ERROR NULL FILENAME");
                    } else {
                        ...
                        if (!file.exists()) { //verifica se o arquivo existe
                            System.out.println("Error: File " + this._path.getRootPath() + "/" + filename + " not found.");
                            out.writeUTF("ERROR FILE NOT FOUND");
                        } else {
                            try {
                                out.writeUTF("SUCCESS"); //informa para o cliente que vai enviar o arquivo
								...
								while ((bytesRead = bufferedInputStream.read(bufferB)) > 0) {
									out.write(bufferB, 0, bytesRead);
									out.flush();
								}

                                System.out.println("File " + filename + " sent to user " + userName);
                            } catch (IOException ioe) {
